...
- 20260112 复现了`getRouters`这个接口，越来越感觉自己是在干复制粘贴的工作了。虽然部分还是做了优化，把菜单表转成树状的功能抽象成了方法，但是大体流程还是一样的，没有仔细去研究每一步到底是在做什么，因为研究了也没有用，最后还是得根据前端需要的数据结构、类型和数据库里存储的内容做适应性调整。到了需要结合前端的地步了。
