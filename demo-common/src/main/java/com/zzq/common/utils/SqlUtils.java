...
public class SqlUtils {
    ...
    public static String escapeOrderBySql(String value)
    {
        ...
        {
            throw new BaseException(ModuleConstants.QUERY, HttpStatus.PARA_ERROR, "参数不符合规范，不能进行查询");
        }
        if (StringUtils.length(value) > ORDER_BY_MAX_LENGTH)
        {
            throw new BaseException(ModuleConstants.QUERY, HttpStatus.PARA_ERROR, "参数已超过最大限制，不能进行查询");
        }
        return value;
    }
    ...
    public static void filterKeyword(String value)
    {
        ...
        for (String sqlKeyword : sqlKeywords) {
            ...
            {
                throw new BaseException(ModuleConstants.QUERY, HttpStatus.PARA_ERROR, "参数存在SQL注入风险");
            }
        }
    }
